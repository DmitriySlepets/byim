<?php
class ControllerPaymentWebPayHalva extends Controller {
	...
	public function index() {
		...
		$data['wsb_test'] = $this->config->get('webpayhalva_test'); // 1 - for test

		if ($data['wsb_test'] == 1)
			$data['action'] = 'https://securesandbox.webpay.by'; // for test
				else
			$data['action'] = 'https://payment.webpay.by/mtb/halva'; // for real
		...
		$this->load->language('payment/webpayhalva');
		...
		$data['success_url'] = $this->config->get('webpayhalva_success_url');
		$data['cancel_url'] = $this->config->get('webpayhalva_fail_url');

		$data['notify_url'] = $this->config->get('webpayhalva_result_url');

		$data['shop_mes'] = $this->config->get('webpayhalva_shop_mes');


		$data['shop_id'] = $this->config->get('webpayhalva_shop_id');
		$data['shop_name'] = $this->config->get('webpayhalva_shop_name');
		$data['order_id'] = $this->session->data['order_id'];
		$data['terminal'] = $this->config->get('webpayhalva_terminal');

		$data['currency_id'] = 'BYN';
		...
		$data['lifetime'] = $this->config->get('webpayhalva_lifetime');

		srand ((int)((double)microtime()*1000003));

		$data['seed'] = rand();

		$data['hash'] = $this->CreateAuthorizationKey ($data['seed'],$data['shop_id'],$data['order_id'],$data['wsb_test'],$data['currency_id'],$data['summ'],$data['terminal'],$this->config->get('webpayhalva_shop_key') );
		...
		foreach ($totals as $total) {
			...
			if ($total['title']=='coupon')
			$data['wsb_discount_price'] = $data['wsb_discount_price'] + abs($total['value']);

			if ($total['title']=='shoputils_cumulative_discounts')
			$data['wsb_discount_price'] = $data['wsb_discount_price'] + abs($total['value']);
    ...
    $data['wsb_tax'] = 0;
		...
		//return $this->load->view('default/template/payment/webpayhalva', $data);
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/webpayhalva.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/webpayhalva.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/webpayhalva.tpl', $data);
		}

	}

	private function CreateAuthorizationKey ($wsb_seed,$wsb_storeid,$wsb_order_num,$wsb_test,$wsb_currency_id,$wsb_total,$wsb_terminal,$SecretKey) { //функция вычисления электронной подписи
		// если версия 2
		$wsb_signature = sha1($wsb_seed.$wsb_storeid.$wsb_order_num.$wsb_test.$wsb_currency_id.$wsb_total.$wsb_terminal.$SecretKey);
	...
	public function confirm() {
		...
		if ($this->session->data['payment_method']['code'] == 'webpayhalva') {
			$this->load->model('checkout/order');

			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('webpayhalva_order_status_progress_id'));
	...
	public function callback() {
		...
		if ($this->CreateSignature($batch_timestamp, $currency_id, $amount, $payment_method, $order_id, $site_order_id, $transaction_id, $payment_type, $rrn, $this->config->get('webpayhalva_shop_key')) == $wsb_signature)
		{
			$this->load->model('checkout/order');
			...
			if ( $order_info) {
				...
				if(($payment_type==1)||($payment_type==4)) {
					...
					if( $order_info['order_status_id'] == 0) {
						$this->model_checkout_order->addOrderHistory($site_order_id, $this->config->get('webpayhalva_order_status_id'), 'webpayhalva');
					}

					if( $order_info['order_status_id'] != $this->config->get('webpayhalva_order_status_id')) {
						$this->model_checkout_order->addOrderHistory($site_order_id, $this->config->get('webpayhalva_order_status_id'),'webpayhalva',TRUE);
					}
				} elseif( ($payment_type==5)||($payment_type==7) ) {
					// не выполнено
					if( $order_info['order_status_id'] == 0) {
						$this->model_checkout_order->addOrderHistory($site_order_id, $this->config->get('webpayhalva_order_status_cancel_id'), 'webpayhalva');

					}

					if( $order_info['order_status_id'] != $this->config->get('webpayhalva_order_status_cancel_id')) {
						$this->model_checkout_order->addOrderHistory($site_order_id, $this->config->get('webpayhalva_order_status_cancel_id'),'webpayhalva',TRUE);
					}
				} else {
					// выполняется
					if( $order_info['order_status_id'] == 0) {
						$this->model_checkout_order->addOrderHistory($site_order_id, $this->config->get('webpayhalva_order_status_progress_id'), 'webpayhalva');
					}

					if( $order_info['order_status_id'] != $this->config->get('webpayhalva_order_status_progress_id')) {
						$this->model_checkout_order->addOrderHistory($site_order_id, $this->config->get('webpayhalva_order_status_progress_id'),'webpayhalva',TRUE);
					}
				...
				{
					$this->log->write('webpayhalva ошибка номера заказа ' . ' ' .http_build_query($this->request->post));
					exit;
				}
		}
		else
			{
				$this->log->write('webpayhalva ошибка подписи ' . ' ' .http_build_query($this->request->post));
				exit;
			}
		// ответить
